...
3. **Read refresh**: four resources have a `Refresh<X>ModelFromResponse` that Read calls after the mapper. `gpcn_gpu` and `gpcn_network` refresh `name`. `gpcn_virtualmachine` refreshes `name` and `size_id`. `gpcn_volume` refreshes `name` and `size_gb`. Each refreshes only attributes that can change out of band and that Terraform can reconcile. Attributes fixed at creation keep the configured value, and Create and Update keep the planned values. `gpcn_ssh_key` and `gpcn_resource_group` have no such function: their shared mapper sets `name` unconditionally, and Create and Update call that mapper too. `gpcn_volume_attachment` refreshes nothing; its Read compares the attached VM id. On `gpcn_volume` both refreshed attributes reconcile only by replacement: `name` requires replacement, and a grow plans a shrink that requires replacement, so an out-of-band rename or grow plans a destroy and create
