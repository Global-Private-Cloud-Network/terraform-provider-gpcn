...
Then delete or rewrite the block. A new block needs `subnet_id` from a `gpcn_vpc_subnet`, and applying it creates a new machine on that subnet. Move data by attaching a `gpcn_volume` to the new machine. A `gpcn_volume_attachment` that references the legacy machine must be removed from state or rewritten with it.
